...
$(document).ready(function(){
	...
	//将加载数据的方法绑定到公共对象
	withdrawFormModule.loadFormList = loadFormList;
...
//		console.info('nowPage:'+nowPage+',count:'+count);
		...
		$("#pagination").pagination(count, {
            'items_per_page'      : withdrawFormModule.paginationParam.pageSize,
            ...
            'callback'            : function(page_index){
            	...
            	if(withdrawFormModule.paginationParam.pageNo != page_index){
            		//重新赋值当前页数
            		withdrawFormModule.paginationParam.pageNo = page_index;
            		//重新载入数据
            		loadFormList(false);
            	}
            },
	...
	function loadFormList(isAll){
		...
		//收集查询控件的参数值
		withdrawFormModule.searchParam = {
			key : $("#fileName").val(),
			startTime : $("#startTime").val(),
			endTime : $("#endTime").val(),
			type : 1,
			pageNo : withdrawFormModule.paginationParam.pageNo,	//请求页
			pageSize : withdrawFormModule.paginationParam.pageSize	//每页记录数
		}
		//返回请求的promise对象
		httpUtilObj.ajax({
			url : CONSTANT.URL_PREFIX + CONSTANT.GET_FORMRECORD_LISG_URL,
			params : withdrawFormModule.searchParam
		}).then(function(data){
			//刷新列表
	...
	function refreshFormReocrdList(data){
		...
		var formWithdrawListHtml = template('formWithdrawListTemplate',data);
		$("#formListContent").empty().html(formWithdrawListHtml);
	...
	$('body').on('click', "#batchDownload",function(){
