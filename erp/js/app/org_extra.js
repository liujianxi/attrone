...
(function (_self) {
	...
	_self.initHos=function(){
		...
		httpUtilObj.ajax({
			url: '/adminjson/SAASTJOrgExtraFee',
			params: data
		}).then((res)=>{//org_hos--
	...
	_self.initTemplate = function (str) {
		let dtd = $.Deferred();
		let httpUtilObj = new HttpUtil();
		let isSelf=$('#org-search').attr('isself')==0?false:true;
		let data = {
			orderType:1,
			startTime:$('input[name="daterange-start"]').val(),
			endTime:$('input[name="daterange-end"]').val(),
	...
	_self.refreshTemplate=function(data){
		var getListHtml = template('orgExtraTemplate', data);
		$("#orgExtraListContent").empty().html(getListHtml);
		_self.refreshBind();
	}
	//更新后的bind事件
	_self.refreshBind = function () {
		$('body').off('click', '.extra-fee').on('click', '.extra-fee', function () {
			let startTime = $('#orgExtraListContent').attr('startTime');
			let endTime = $('#orgExtraListContent').attr('endTime');
			...
			let data = {
				...
				endTime: endTime,
				startTime: startTime,
				payBusiType: $(this).attr('payBusiType'),
				orgId:orgId,
				branchId:branchId,
			};
		...
		$('.export').off('click').on('click', function () {
			...
			httpUtilObj.ajax({
				url: '/adminjson/SAASGetFormExtraFee',
				params: data,
			}).then((res)=>{
				output_flag=true;
				if(res.errorCode==0){
