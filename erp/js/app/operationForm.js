...
(function (_self) {
	...
	_self.initData = function () {
		...
		$('.operation-form li span').off('click').on('click',function(){
			$('.operation-form li span').removeClass('selected');
			$(this).addClass('selected');
			$('.form-box').addClass('org_hide');
			$('.form-box').eq($(this).attr('value')-1).removeClass('org_hide');
	...
	_self.initTemplate = function (num=1,str) {
		...
		let data={};
		data={//1-提现 2-损坏 3-过期 6-归还
			key : $("#fileName").val(),
			startTime : $("#startTime").val(),
			endTime : $("#endTime").val(),
			type : $('.operation-form li span.selected').attr('type'),
			pageNo : num,	//请求页
	...
	_self.refreshTemmlate=function(data){
		...
		//1-门禁卡报表,2-归还，3-过期，4-作废，5-租借
		let value=$('.operation-form li span.selected').attr('value');
		if(value==1){
			let t_Html = template('extraFormListTemplate', data);
			$("#extraformListContent").empty().html(t_Html);
		}else if(value==2){
			let a_Html = template('returnFormListTemplate', data);
			$("#returnformListContent").empty().html(a_Html);
		}else if(value==3){
			let b_Html = template('pastFormListTemplate', data);
			$("#pastFormListContent").empty().html(b_Html);
		}else if(value==4){
			let c_Html = template('damagedFormListTemplate', data);
			$("#damagedformListContent").empty().html(c_Html);
		}else if(value==5){
