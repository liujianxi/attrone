...
$(document).ready(function(){
	...
	//将加载数据的方法绑定到公共对象
	insureApplyFormModule.insureApplyFormList = insureApplyFormList;
	
	//实例化请求帮助类
	var httpUtilObj = new HttpUtil();
	//默认载入数据
	insureApplyFormList(true,1);
...
//		console.info('nowPage:'+nowPage+',count:'+count);
		//如果记录条目为0则隐藏分页控件
		if(count <=insureApplyFormModule.paginationParam.pageSize){
			$("#pagination").hide();
		}else{
			$("#pagination").show();
		}
		...
		$("#pagination").pagination(count, {
            'items_per_page'      : insureApplyFormModule.paginationParam.pageSize,
            ...
            'callback'            : function(page_index){
            	...
            	if(insureApplyFormModule.paginationParam.pageNo != page_index){
            		//重新赋值当前页数
            		insureApplyFormModule.paginationParam.pageNo = page_index;
            		//重新载入数据
            		insureApplyFormList(false);
            	}
            },
	 ...
	 */
	function insureApplyFormList(isAll){
		//1-提现 2-作废 3-过期 4-长护险复审名单
		//收集查询控件的参数值
		insureApplyFormModule.searchParam = {
			key : $("#fileName").val(),
			startTime : $("#startTime").val(),
			endTime : $("#endTime").val(),
			type : 4,
			pageNo : insureApplyFormModule.paginationParam.pageNo,	//请求页
			pageSize : insureApplyFormModule.paginationParam.pageSize	//每页记录数
		}
		//返回请求的promise对象
		httpUtilObj.ajax({
			url : CONSTANT.URL_PREFIX + CONSTANT.GET_FORMRECORD_LISG_URL,
			params : insureApplyFormModule.searchParam
		}).then(function(data){
			//刷新列表
	...
	function refreshFormReocrdList(data){
		...
		var forminsureApplyListHtml = template('forminsureApplyListTemplate',data);
		$("#formListContent").empty().html(forminsureApplyListHtml);
	...
	$('body').on('click', "#damagedDowninsureApply",function(){
...
//			StartGETRequest($(this).val(), function(){});
    ...
    function _createIFrame(url, triggerDelay, removeDelay) {
        ...
        setTimeout(function() {
            var frame = $('<iframe style="display: none;" class="multi-downinsureApply"></iframe>');
            frame.attr('src', url);
