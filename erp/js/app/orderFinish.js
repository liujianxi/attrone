...
(function (_self) {
	...
	_self.initHos=function(){
		...
		httpUtilObj.ajax({
			url: '/adminjson/SAASTJOrderFinish',
			params:data,
		}).then((res)=>{//org_hos--
	...
	_self.refreshTemmlate=function(data){
		var getListHtml = template('orderFinishListTemplate', data);
		$("#orderFinishListContent").empty().html(getListHtml).attr({
			'startdate':$('input[name="daterange-start"]').val(),
			'enddate':$('input[name="daterange-end"]').val(),
		});
	}
	...
	_self.refreshBind = function (data) {
		$('.finish-detail').off('click').on('click',function(){
			let data = {
    			startDate:$("#orderFinishListContent").attr('startdate'),
    			endDate:$("#orderFinishListContent").attr('enddate'),
    			orgId:$(this).attr('orgid'),
			};
			let title='订单结束统计-'+$(this).attr('orgName');
