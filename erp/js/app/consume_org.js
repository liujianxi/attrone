...
(function (_self) {
	...
	_self.initTemplate = function (str) {
		...
		data['companyId']=$('.org_company option:selected').attr('id')||0;
	...
	_self.refreshTemmlate=function(data){
		...
		$("#consumeListContent").empty().html(getListHtml);
		_self.refreshBind();
	}
	//更新后的bind事件
	_self.refreshBind = function () {
		$('body').off('click','.consume_open').on('click','.consume_open',function(){
			let startDate=$(this).attr('startDate');
    		let endDate=$(this).attr('endDate');
    		let title="消费明细"+startDate+'-'+endDate;
    		let data = {
    			endDate:endDate,
    			startDate:startDate,
    			orderType:1,
    		...
    		let order_link="templates/consumeOrg_detail.html";
