...
(function (_self) {
	...
	_self.initData = function () {
		...
		//搜索
		$('#orderHG-search').off('click').on('click',function(){
	...
	_self.initHos=function(){
		...
		httpUtilObj.ajax({
			url: '/adminjson/SAASTJOrderHgFee',
			params: data
		}).then((res)=>{//org_hos--
	...
	_self.refreshTemplate=function(data){
		var getListHtml = template('hgFeeTemplate', data);
		$("#hgFeeListContent").empty().html(getListHtml);
		_self.refreshBind();
	}
	//更新后的bind事件
	_self.refreshBind = function () {
		...
		$('.export').off('click').on('click', function () {
			...
			httpUtilObj.ajax({
				url: '/adminjson/SAASGetFormOrderHgFee',
				params: data,
			}).then((res)=>{
				output_flag=true;
				if(res.errorCode==0){
