...
$(document).ready(function() {
	...
	nurseModule.selctChange = selctChange;
	nurseModule.loadHGList = loadHGList;
	...
	function loadBranchList4Select() {
		...
		httpUtilObj.ajax({
			...
			var oldnumber = nurseModule.oldnumber;
	...
	nurseModule.addHBRelation = function() {
		addHBPanel.show();
	}
	nurseModule.ajaxFileUpload = function(prefix, type, fileId) {
		$.ajaxFileUpload({
	...
	nurseModule.buildBranchList = function() {
	...
	nurseModule.saveHGItem = function() {
		...
		httpUtilObj.ajax({
			...
			if(result.errorCode == 0) {
				...
				if(nurseModule.id > 0)
					nurseModule.loadHGList(false, 1);
				else
					nurseModule.loadHGList(true, 1);
				hgInfoModal.hide();
			}
		});
	}

	nurseModule.changeRoleDiv = function() {
	...
	nurseModule.editHGItem = function() {
	...
	nurseModule.cancelHGEdit = function() {
		$("#cancelBtn").addClass('hidden');
	...
	nurseModule.delHGItem = function(id) {
		bootbox.confirm({
			title: "系统提示",
	...
	nurseModule.showEditHG = function(actionType, id, name) {
		
		
		nurseModule.actionType = actionType;
		console.log(actionType);
		if(actionType == 'add') {
			top.importOnceJS('js-script-nurse',"js/app/rp/nurse.js");
			top.G_Fun_showAddnursePanel().then(function(data){
				loadHGList(false, 1);
			});
		}else if(actionType == 'edit') {
			top.importOnceJS('js-script-nurse',"js/app/rp/nurse.js");
			top.G_Fun_shownurseInfoPanel(id).then(function(data){
				loadHGList(false, 1);
			});
		}
...
//			nurseModule.id = 0;
//			nurseModule.changeRoleDiv();
//			content2Edit();
//			$("#saveHGBtn").removeClass('hidden');
//			$("#cancelBtn").removeClass('hidden');
//			$("#editHGBtn").addClass('hidden');
//			$("#cancelHGBtn").addClass('hidden');
//			nurseModule.oldnumber = [];
...
//					nurseModule.oldnumber = data.branchIds;
...
//					$("#searchBut").attr("disabled", false);
				...
				if($("#search_org").val() && $("#search_org").val() != '') {
					if(!nurseModule.paginationParam.isInit) {
						nurseModule.loadHGList(true, 1);
						nurseModule.paginationParam.isInit = true;
					}
				}
			}
		});
	...
	function getHGListData(pageNo) {
		...
		//收集查询控件的参数值
		nurseModule.searchParam = {
			hgType : 10002,
			orgId: $("#search_org").val(),
			key: $("#findHgName").val(),
			pageNo: pageNo, //请求页
			pageSize: nurseModule.paginationParam.pageSize //每页记录数
		}
		//返回请求的promise对象
		return httpUtilObj.ajax({
			url: CONSTANT.URL_PREFIX + CONSTANT.GET_HG_LISG_URL,
			params: nurseModule.searchParam
		});
	}
	/**
	 * 初始化搜索控件
	...
	function initSearchWidget() {
		...
		dataPromise.then(function(result) {
			nurseModule.provinceList = result;
	...
	function selctChange(type, e) {
		...
		//根据类型处理子级下拉框的加载
		if(type == 'pro' && code != '') {
			console.info(nurseModule.provinceList);
			var cityList = getCityJSON(code, nurseModule.provinceList).city;
			nurseModule.cityList = cityList;
			initSelct('city', cityList);
			initSelct('distrct', []); //清空区县下拉框
		} else if(type == 'city' && code != '') {
			var selectedCity = getCityJSON(code, nurseModule.cityList);
	...
	function initPagination(nowPage, count) {
		...
		$("#pagination").pagination(count, {
			'items_per_page': nurseModule.paginationParam.pageSize,
			...
			'callback': function(page_index) {
				...
				//当前页数
				nurseModule.paginationParam.pageNo = page_index + 1;
			},
	 ...
	 */
	function loadHGList(isAll, pageNo) {
		//请求数据
